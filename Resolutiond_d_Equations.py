...
def Banach(func,M,a0,b0,nIterations):
    ...
    #Sortie : void.
    #Résultat : constrution de liste contenant les valeurs de la suite itérative.
    # Attention, cette routine cherche le point zéro de la fonction entrée avec la méthode de Banach ! ...
    # ... si l'on veux le point fixe d'une fonction f(x), il faut entrer -x+f(x) pour func.
    # Conditions : Intervalle fermé ; fonction monotone, fonction m-infra-Lipschizienne, M-supra-Lipschitzienne ; contraste de signe
    
    print("Point fixe de Banach")
    print("La fonction est supposée CROISSANTE et la pente de toute corde sur sa courbe est bornée inférieurement et supérieurement")
...
def Secante(func,x0,x1):
    ...
    while (func(x[i-1])-func(x[i-2]) != 0): #C'est-à-dire tant que les deux valeurs ne sont pas trop proches pour le compilateur
        #Equation itérative caractéristique
        pr=(x[i-1]-x[i-2])/(func(x[i-1])-func(x[i-2])) #Pente renversée
        x.append(x[i-1]-(func(x[i-1])*pr))
        print(str(i)+"\t"+str(x[i])) #Sortie
        i = i+1
